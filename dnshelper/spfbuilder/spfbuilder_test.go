...
func TestBuildSPFRecord(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: map[string][]string{
				"@":                {"v=spf1 ip4:192.168.0.1/32 ip4:192.168.0.10/32 ip4:192.168.0.100/32 include:spf1.example.org ~all"},
				"spf1.example.org": {"v=spf1 ip4:192.168.0.2/32 ip4:192.168.0.200/32 ip4:192.168.0.3/32 include:spf2.example.org ~all"},
				"spf2.example.org": {"v=spf1 ip4:192.168.0.4/32 ip4:192.168.0.5/32 ip4:192.168.0.6/32 include:spf3.example.org ~all"},
				"spf3.example.org": {"v=spf1 ip4:192.168.0.7/32 ip4:192.168.0.8/32 ip4:192.168.0.9/32 ip6:fe80:831e:c000::/38 ~all"},
			},
			wantErr: false,
		},
		{
			...
			flatten:           []string{"example.org", "_spf.example.org"},
			want: map[string][]string{
				"@":    {"v=spf1 ip4:192.168.0.1/32 ip4:192.168.0.10/32 ip4:192.168.0.100/32 ip4:192.168.0.2/32 ip4:192.168.0.200/32 ip4:192.168.0.3/32 ip4:192.168.0.4/32 ip4:192.168.0.5/32 ip4:192.168.0.6/32 ip4:192.168.0.7/32 ip4:192.168.0.8/32 include:spf1.example.org ~all"},
				"spf1": {"v=spf1 ip4:192.168.0.9/32 ip6:fe80:831e:c000::/38 ~all"},
			},
			wantErr: false,
		},
		{
		...
		{
			name:              "stable ordering of flattened parts",
			domain:            "example.com",
			overflow:          "spf%d",
			txtMaxSize:        255,
			domainOnRecordKey: false,
			parts:             []string{"v=spf1", "include:_spf.example-unsorted.com", "-all"},
			flatten:           []string{"_spf.example-unsorted.com"},
			want: map[string][]string{
				"@": {"v=spf1 ip4:192.168.1.1/32 ip4:192.168.1.3/32 ip4:192.168.1.5/32 ip4:192.168.1.9/32 -all"},
			},
			wantErr: false,
		},
	}
